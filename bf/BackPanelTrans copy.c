...
#endif
uint16_t _spiITTimeOut = 10; // SPI IT消息超时时间
uint16_t _SYNCTimeOut = 10; // 同步信号超时时间
uint16_t _waitCSTimeOut = 10;
...
BackPanelTransStatus_TypeDef BackPanel_TransferDATA(uint8_t *writeData, uint8_t *readData, uint8_t DataLen)
{
#if 0
	LOG("BackPanel_TransferDATA : ");
	for (uint16_t i = 0; i < DataLen; i++)
	{
		if (HAL_SPI_TransmitReceive_IT(&BackPanelTransPort, (writeData+i), (readData+i), 1) != HAL_OK)
			return bpTrans_SPI_Err;
		if (waitSPISTATEready() != bpTrans_OK)	return bpTrans_SPIIT_Err;
		LOG("%d ", (*(readData+i)));
	}
...
#endif
#if 1
	if (HAL_SPI_TransmitReceive_IT(&BackPanelTransPort, writeData, readData, DataLen) != HAL_OK)
		return bpTrans_SPI_Err;
	if (waitSPISTATEready() != bpTrans_OK)	return bpTrans_SPIIT_Err;
#endif
	return bpTrans_OK;
 ...
 * @param  DataLen ： master板有效数据的长度
 * @retval
 */
BackPanelTransStatus_TypeDef BackPanelTrans_Master_readDataFrom_Slave(BoardID_TypeDef currentID, uint8_t *writeData, uint8_t *readData, uint8_t DataLen)
{
	BackPanelTransStatus_TypeDef bpstatus;
	/*先将数据打包好*/
	//Packet_ConstructPacket(currentID, writeData, DataLen, pack_writeData,  &packwLen);
	Packet_ConstructPacket(currentID, writeData, &DataLen);	
	uint8_t packetreadData[DataLen];	//临时的接收数据的数组

	#if 1
	LOG("......packeted write data : ");
	for (uint16_t i = 0; i < DataLen; i++)
	{
		LOG("%d ", writeData[i]);	//wData[i]
	}
	LOG("\r\n...wDataLen : %d\r\n", DataLen);
	#endif
	...
	/* 2------主控板发送 同步信号sync signal 给 slave板 */
	...
	/* 3------从slave板接收数据------------ */
	BackPanel_TransferDATA(writeData, packetreadData, DataLen);
	SPI1_CS_DISABLE(currentID);
	free(writeData);
	if (waitSPISTATEready() != bpTrans_OK)	return bpTrans_SPIIT_Err;	//等待SPI可用
#if 1
	LOG("r data from slave : ");
	for (int i = 0; i < DataLen; i++)
	{
		if (_printDebug) LOG("%d ", packetreadData[i]);
	}
	if (_printDebug) LOG("\r\n.......%d\r\n", DataLen);
#endif

	/* 4------解包接收到的数据------------ */
	PacketStatus_TypeDef pstate = Packet_UnpackPacket(currentID, packetreadData, &DataLen);	//packwLen, act_readData, &actrLen);
	LOG("packet state : %d\r\n", pstate);
#if 1
	LOG("Packet_UnpackPacket read data : ");
	for (int i = 0; i < DataLen; i++)
	{
		readData[i] = packetreadData[i];
		if (_printDebug) LOG("%d ", packetreadData[i]);
	}
	if (_printDebug) LOG("\r\n.......%d\r\n", DataLen);
#endif
	free(packetreadData);
	if (pstate != PACKET_OK) return bpTrans_Packet_Err;
	if (_printDebug) LOG("end read data from slave ............%ld\r\n", xTaskGetTickCount());
	return bpstatus;
}
#if 0
