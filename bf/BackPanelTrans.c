...
BackPanelTransStatus_TypeDef BackPanel_TransferDATA(uint8_t *writeData, uint8_t *readData, uint16_t DataLen)
{
#if 0
 ...
 * @retval
 */
BackPanelTransStatus_TypeDef BackPanelTrans_Master_readDataFrom_Slave(BoardID_TypeDef currentID, uint8_t *writeData, uint8_t *readData, uint16_t DataLen)
{
	BackPanelTransStatus_TypeDef bpstatus;
	...
	BackPanel_TransferDATA(writeData, readData, DataLen);
	if (waitSPISTATEready() != bpTrans_OK)	return bpTrans_SPIIT_Err;	//等待SPI可用
