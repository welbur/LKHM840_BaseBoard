	...
	for (;;)
	{
		...
		for (int i = 0; i < PowerBoardNum; i++)
		{
			if (PowerBoard_Trig[i])
			{
				...
				uint16_t DataLen = 126;
				uint8_t wData[DataLen];
				uint8_t rData[DataLen];
				
				BackPanelTransStatus_TypeDef bpstatus = bpTrans_Wait;
				//HAL_SPI_MspInit(&hspi1);
				bpstatus = BackPanelTrans_Master_readDataFrom_Slave(i, wData, rData, DataLen);
				//HAL_SPI_MspDeInit(&hspi1);
				LOG("current board %d status : ..%d..\r\n", i, bpstatus);
				LOG("\r\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ end time : %ld ~~~~~~~~~~%ld\r\n\r\n", xTaskGetTickCount(), ++RPBD_Act_Countt);
 ...
 * @brief modbus指令格式 ： _______________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________	
 ...
 * 			  |_____________________________________________________________________|___________________________________________________________________________________|__________________________________________________________________________|________________________________________________________________________________________|
 * @param argument: Not used
 * @retval None
 */
#if 1
