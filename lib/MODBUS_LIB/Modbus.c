...
#else
			for (int i = 0; i < 17; i++) modH->FCStatus[i] = 0;
			modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusSlave, modH, &myTaskModbusA_attributes);
#endif
		}
...
{
	// check message crc vs calculated crc
	// LOGW("check message crc vs calculated crc \r\n");
#if ENABLE_TCP == 1
	uint16_t u16MsgCRC;
	...
	case MB_FC_WRITE_MULTIPLE_COILS:
		...
		// u16AdRegs = (u16AdRegs - FC15_AddrOffset) / 16;
